...
func TestE2e(t *testing.T) {
	...
	if !slices.Contains(test.LocalCloudProviders, test.VeleroCfg.CloudProvider) {
		...
		assert.NotEqualf(t, "", test.VeleroCfg.ObjectStoreProvider, "No object store provider specified - must be specified when using kind as the cloud provider") // Must have an object store provider
	}
